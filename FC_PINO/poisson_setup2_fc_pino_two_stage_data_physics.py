# %% [markdown]
# # FC_PINO two-stage data + physics baseline for `poisson_setup2`
#
...
# 2. Continue from that model with a combined stage-2 loss: physics loss on the first `num_physics_train` (`N_2`) parameter fields, and supervised data loss on a separately sampled batch from the first `N_1` labeled fields. Here `N_2 > N_1` and the physics set includes the supervised set.
#
...
model_train_outputs_path = repo_path / 'results/poisson_setup2/model_train_outputs/fc_pino_two_stage_data_physics'
model_test_outputs_path = repo_path / 'results/poisson_setup2/model_test_outputs/fc_pino_two_stage_data_physics'
...
# fc_degree must stay low: the FC extension is a high-degree boundary polynomial
# extrapolation, and on the rough sum-of-Gaussians p field its magnitude grows ~10x
# per degree (d=2->6, d=3->2e2, d=6->3e7). d=6 blew up the extended input to ~1e7 and
# stalled training; d=3 keeps it bounded and the loss decays normally.
# Trade-off (physics is active here, physics_weight>0): lower d also lowers FC
# derivative accuracy. If high-d derivatives are needed, prefer the FC_FNO_end model,
# which extends only for derivative computation and not for the forward/lifting.
fc_degree = 2
fc_cont_points = 50
n_modes = (32, 32)
hidden_channels = 128
...
assert num_x == num_y, 'This notebook assumes the square poisson_setup2 grid used by the existing FNO baseline.'
...
def scalar_vertices_to_xy(values: np.ndarray) -> np.ndarray:
    # For poisson_setup2 the p vertex arrays are scattered through the same
    # perm / meshgrid(x, y).T order as the sigma/u arrays (x-outer, y-inner; see
    # poisson_permeability_setup2_evaluating_grid_points.py), so a plain C-order
...
#
# The continuous residual uses the same `q`, `w`, and `f2` definitions as `PoissonSetup2LeastSquares`.
# For setup 2, `f1 = 0` and `f2` is a sum of Gaussians evaluated on the vertex grid.
...
image_mesh_coords = np.array(np.meshgrid(x_grid, y_grid)).T.reshape(-1, 2)  # Match poisson_setup2_fno.ipynb flat vertex order.
...
# f1 = 0 for setup 2.
# f2 is a sum of Gaussians centered at (m/4, n/4) for m, n in {1, 2, 3}.
X, Y = np.meshgrid(x_grid, y_grid, indexing='ij')
w_f2 = 1.0 / 32.0
centers_f2 = [(m / 4.0, n / 4.0) for m in (1, 2, 3) for n in (1, 2, 3)]
f2_xy = np.zeros_like(X, dtype=np.float64)
for cx, cy in centers_f2:
    f2_xy += 100.0 * np.exp(-((np.sqrt((X - cx) ** 2 + (Y - cy) ** 2) / w_f2) ** 2))
...
f2_tensor = torch.as_tensor(f2_xy[None, None, :, :], dtype=torch.float64, device=device)
print('q_grad:', tuple(q_grad_tensor.shape), 'w_grad:', tuple(w_grad_tensor.shape), 'f2:', tuple(f2_tensor.shape))
...
def fc_residual_terms(p, pred, derivs):
    ...
    # f1 = 0 for setup 2.
    r1_x = sigma_x - (p * u_x + p * w_grad_tensor[:, 0:1] - q_grad_tensor[:, 0:1])
    r1_y = sigma_y - (p * u_y + p * w_grad_tensor[:, 1:2] - q_grad_tensor[:, 1:2])
    r2 = div_sigma + f2_tensor
...
for i in tqdm(range(num_test), desc='Build CG1 predictions/labels and project predictions'):
    ...
    # Match poisson_setup2_fno.ipynb: pack collapsed CG1 sigma/u into the mixed
...
# ## Visualizations
#
# Parameter images and FEM-evaluated reference/prediction/difference plots for the projected solution fields, following `poisson_setup2_fno.ipynb`.
#
...
for i in range(num_visualize_actual):
    ...
    plt.title('parameter (setup 2)', fontsize=20)
    plt.xticks([])
    plt.yticks([])
