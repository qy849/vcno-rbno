...
try:
    ...
    class FourierDiff:
        def __init__(self, dim, L):
            ...
            if len(self.L) != dim:
                raise ValueError(f"Expected {dim} domain lengths, got {len(self.L)}")

        def _freqs(self, x):
            spatial_shape = x.shape[-self.dim:]
            freqs = []
            for n, length in zip(spatial_shape, self.L):
                spacing = length / n
                k = 2 * torch.pi * torch.fft.fftfreq(n, d=spacing, device=x.device, dtype=x.real.dtype)
                freqs.append(k)
            return freqs

        def _differentiate(self, x, derivative):
            if self.dim == 1 and isinstance(derivative, int):
                derivative = (derivative,)
            derivative = tuple(int(v) for v in derivative)
            if len(derivative) != self.dim:
                raise ValueError(f"Expected derivative tuple of length {self.dim}, got {derivative}")

            spatial_dims = tuple(range(x.ndim - self.dim, x.ndim))
            ...
            for axis, order in enumerate(derivative):
                if order == 0:
                    continue
                shape = [1] * x.ndim
                shape[x.ndim - self.dim + axis] = freqs[axis].numel()
                k = freqs[axis].reshape(shape)
                multiplier = multiplier * (1j * k) ** order
            out = torch.fft.ifftn(x_hat * multiplier, dim=spatial_dims)
            return out.real if not torch.is_complex(x) else out

        def compute_multiple_derivatives(self, x, derivatives):
            return [self._differentiate(x, derivative) for derivative in derivatives]

        def dx(self, x, order=1):
            if self.dim != 1:
                raise ValueError('dx(order=...) is only defined for dim=1; use compute_multiple_derivatives otherwise.')
            return self._differentiate(x, (order,))
try:
...
class FC_FNO_end(FNO):

    """
    FC_FNO_end is the same as FC_FNO except that it does not apply Fourier Continuation to the input before the lifitng layer, 
    but rather only when computing the derivatives.

    See the FNO or FC_FNO classes for more details on implementation.
    ...
    def __init__(
        ...
        # Convert FNO block weights to complex double precision for higher precision
        for i in range(n_layers):
            self.fno_blocks.convs[i].weight = self.fno_blocks.convs[i].weight.to(
                torch.cdouble
            )
        ...
        self.projection_nonlinearity = projection_nonlinearity
        self.Lengths = Lengths
        self.derivs_to_compute = derivs_to_compute
    ...
    def dQ_1D(self, X1, Dx_arr, Q1, Q2):
        ...
        need_dx = "dx" in self.derivs_to_compute or "dxx" in self.derivs_to_compute
        if need_dx:
            dx = Dx_arr["dx"]
        if "dxx" in self.derivs_to_compute:
            dxx = Dx_arr["dxx"]
        ...
        if "dx" in self.derivs_to_compute:
            Dx = torch.einsum("boix,bix->box", dQ, dx)
            Dx_out.append(Dx)
    ...
    def dQ_2D(self, X1, Dx_arr, Q1, Q2):
        """
        ...
        ***** Assumes that the final nonlinearity is tanh. ******
        ...
        # b: batch size
        # m: hidden/intermediate dimension
        # i: input dimension
        # o: output dimension
        # t: time dimension
        # x: spatial dimension (x-coordinate)
        # y: spatial dimension (y-coordinate)
        #
        # TENSOR SHAPES:
        # dW1: first projection layer weights (m, i)
        # dW2: final projection layer weights (m, o) - transposed from (o, m)
        # X1: intermediate representation after first projection (b, m, t, x)
        # Dtanh: derivative of tanh activation (b, m, t, x)
        # dQ: Jacobian of the composed function (b, o, i, t, x)
        # H2: Hessian of final projection at intermediate point (b, o, m, t, x)
        # wx, wy: spatial and time derivatives (b, i, t, x)
        # wxx, wyy: second spatial derivatives (b, i, t, x)

        Dx_out = []
        need_dx = "dx" in self.derivs_to_compute or "dxx" in self.derivs_to_compute
        need_dy = "dy" in self.derivs_to_compute or "dyy" in self.derivs_to_compute
        if need_dx:
            wx = Dx_arr["dx"]
        if need_dy:
            wy = Dx_arr["dy"]
        ...
        if self.projection_nonlinearity == F.tanh:
            ...
            raise ValueError(
                f"Projection nonlinearity {self.projection_nonlinearity} not supported. Must be F.tanh or F.sigmoid"
            )
        ...
        # wxQ: spatial/x-derivative of output with respect to input
        if "dx" in self.derivs_to_compute:
            wxQ = torch.einsum("boitx,bitx->botx", dQ, wx)
            Dx_out.append(wxQ)

        # wyQ: time/y-derivative of output with respect to input
        if "dy" in self.derivs_to_compute:
            wyQ = torch.einsum("boitx,bitx->botx", dQ, wy)
            Dx_out.append(wyQ)

        ### Second Derivative: D²(f∘g) = D²f(g) · (Dg)² + Df(g) · D²g (Chain Rule)

        if "dxx" in self.derivs_to_compute:
            wxx = Dx_arr["dxx"]
        if "dyy" in self.derivs_to_compute:
            wyy = Dx_arr["dyy"]

        # This is the analytic second derivative: d²/dx²[tanh(x)] = -2*sech²(x)*tanh(x)
        if self.projection_nonlinearity == F.tanh:
            dP2 = -2 * dP1 * torch.tanh(X1)
        elif self.projection_nonlinearity == F.silu:
            dP2 = (
        ...
        if "dyy" in self.derivs_to_compute:
            # wyy1: first term of chain rule: J_g^T · H_f · J_g
            wyy1 = torch.einsum("bitx,mi,bomtx,mj,bjtx->botx", wy, dW1, H2, dW1, wy)
            # wyy2: second term of chain rule: Df(g) · D²g
            wyy2 = torch.einsum("boitx,bitx->botx", dQ, wyy)
            # Combine both terms: D²(f∘g) = D²f(g) · (Dg)² + Df(g) · D²g
            wyyQ = wyy1 + wyy2
            Dx_out.append(wyyQ)
    ...
    def dQ_3D(self, X1, Dx_arr, Q1, Q2):
        """
        Chain rule of model, call it Q, to compute dQ/d(inputs). Uses einsum for efficiency.
        ...
        ***** Assumes that the final nonlinearity is tanh. ******
        ...
        need_dx = "dx" in self.derivs_to_compute or "dxx" in self.derivs_to_compute
        need_dy = "dy" in self.derivs_to_compute or "dyy" in self.derivs_to_compute
        need_dz = "dz" in self.derivs_to_compute or "dzz" in self.derivs_to_compute
        
        dx = Dx_arr["dx"] if need_dx else None
        dy = Dx_arr["dy"] if need_dy else None
        dz = Dx_arr["dz"] if need_dz else None

        dxx = Dx_arr["dxx"] if "dxx" in self.derivs_to_compute else None
        dyy = Dx_arr["dyy"] if "dyy" in self.derivs_to_compute else None
        dzz = Dx_arr["dzz"] if "dzz" in self.derivs_to_compute else None
        ...
        if "dz" in self.derivs_to_compute:
            wzQ = torch.einsum("boitxz,bitxz->botxz", dQ, dz)
            Dx_out.append(wzQ)
        ...
        if "dyy" in self.derivs_to_compute:
            ...
            wyyQ = wyy1 + wyy2
            Dx_out.append(wyyQ)
        if "dzz" in self.derivs_to_compute:
            wzz1 = torch.einsum("bitxz,ci,bcotxz,cj,bjtxz->botxz", dz, dW1, H2, dW1, dz)
            wzz2 = torch.einsum("boitxz,bitxz->botxz", dQ, dzz)
            wzzQ = wzz1 + wzz2
            Dx_out.append(wzzQ)
    ...
    def forward(self, x, output_shape=None, **kwargs):
        ...
        for layer_idx in range(self.n_layers):
            x = self.fno_blocks(
                x,
                layer_idx,
                output_shape=output_shape[layer_idx],
            )

        ## Compute derivatives if end or start
        if self.derivs_to_compute is not None:
            Dx_arr = {}
            deriv_tuples = []
            if self.n_dim == 1:
                x1 = self.FC_obj(x, dim=1)
                new_Lengths = self.Lengths[0] * (x_res + self.n_additional_pts) / x_res
                FourierDiff1d = FourierDiff(dim=self.n_dim, L=new_Lengths)
                if "dx" in self.derivs_to_compute:
                    dx_tuple = 1
                    deriv_tuples.append(dx_tuple)
                if "dxx" in self.derivs_to_compute:
                    dxx_tuple = 2
                    deriv_tuples.append(dxx_tuple)
                deriv_array = FourierDiff1d.compute_multiple_derivatives(
                    x1, derivatives=deriv_tuples
                )
                for i, deriv_tuple in enumerate(deriv_tuples):
                    if deriv_tuple == (1):
                        Dx_arr["dx"] = self.FC_obj.restrict(deriv_array[i], 1)
                ...
                x2 = self.FC_obj(x, dim=2)
                new_Lengths = (
                    self.Lengths[0] * (x_res + self.n_additional_pts) / x_res,
                    self.Lengths[1] * (y_res + self.n_additional_pts) / y_res,
                )
                FourierDiff2d = FourierDiff(dim=self.n_dim, L=new_Lengths)
                if "dy" in self.derivs_to_compute:
                    dy_tuple = (0, 1)
                ...
                if "dxx" in self.derivs_to_compute:
                    dxx_tuple = (2, 0)
                    deriv_tuples.append(dxx_tuple)
                deriv_array = FourierDiff2d.compute_multiple_derivatives(
                    x2, derivatives=deriv_tuples
                )
                for i, deriv_tuple in enumerate(deriv_tuples):
                    if deriv_tuple == (0, 1):
                        Dx_arr["dy"] = self.FC_obj.restrict(deriv_array[i], 2)
                ...
                x3 = self.FC_obj(x, dim=3)
                new_Lengths = (
                    self.Lengths[0] * (x_res + self.n_additional_pts) / x_res,
                    self.Lengths[1] * (y_res + self.n_additional_pts) / y_res,
                    self.Lengths[2] * (z_res + self.n_additional_pts) / z_res,
                )
                FourierDiff3d = FourierDiff(dim=self.n_dim, L=new_Lengths)
                if "dz" in self.derivs_to_compute:
                    dz_tuple = (0, 0, 1)
                ...
                if "dzz" in self.derivs_to_compute:
                    dzz_tuple = (0, 0, 2)
                    deriv_tuples.append(dzz_tuple)
                deriv_array = FourierDiff3d.compute_multiple_derivatives(
                    x3, derivatives=deriv_tuples
                )
                for i, deriv_tuple in enumerate(deriv_tuples):
                    if deriv_tuple == (0, 0, 1):
                        Dx_arr["dz"] = self.FC_obj.restrict(deriv_array[i], 3)
            ...
            if self.n_dim == 1:
                X1 = Q1(x.transpose(1, 2))
                Dx_arr = self.dQ_1D(X1, Dx_arr, Q1, Q2)
            elif self.n_dim == 2:
                X1 = Q1(x.permute(0, 2, 3, 1))
                Dx_arr = self.dQ_2D(X1, Dx_arr, Q1, Q2)
            elif self.n_dim == 3:
                X1 = Q1(x.permute(0, 2, 3, 4, 1))
                Dx_arr = self.dQ_3D(X1, Dx_arr, Q1, Q2)
            else:
                raise ValueError(
                    f"Error: expected 1, 2, or 3 dimensions, got {self.n_dim}"
                )
...
class FC_FNO_inandout(FNO):
    """
    FC_FNO_inandout is the same as FC_FNO except that it does not use the chain rule to compute the derivatives, but rather 
    computes the derivatives after all the layers are applied.
    ...
    def forward(self, x, output_shape=None, **kwargs):
        ...
        if self.derivs_to_compute is not None:
            ...
            if self.n_dim == 1:
                ...
                for i, deriv_tuple in enumerate(deriv_tuples):
                    if deriv_tuple == (1):
                        Dx_arr.append(self.FC_obj.restrict(deriv_array[i], 1))
                    elif deriv_tuple == (2):
                        Dx_arr.append(self.FC_obj.restrict(deriv_array[i], 1))
                ...
                for i, deriv_tuple in enumerate(deriv_tuples):
                    if deriv_tuple == (0, 1):
                        Dx_arr.append(self.FC_obj.restrict(deriv_array[i], 2))
                    elif deriv_tuple == (1, 0):
                        Dx_arr.append(self.FC_obj.restrict(deriv_array[i], 2))
                    elif deriv_tuple == (0, 2):
                        Dx_arr.append(self.FC_obj.restrict(deriv_array[i], 2))
                ...
                for i, deriv_tuple in enumerate(deriv_tuples):
                    if deriv_tuple == (0, 0, 1):
                        Dx_arr.append(self.FC_obj.restrict(deriv_array[i], 3))
                    elif deriv_tuple == (0, 1, 0):
                        Dx_arr.append(self.FC_obj.restrict(deriv_array[i], 3))
                    elif deriv_tuple == (1, 0, 0):
                        Dx_arr.append(self.FC_obj.restrict(deriv_array[i], 3))
...
class FC_FNO_out(FNO):

    """

    FC_FNO_out is the same as a normal FNO except that after all layers are applied, 
    it applies Fourier Continuation to compute the derivatives.

    See FC_FNO or FNO classes for more details on implementation.
