# %% [markdown]
# # FC_PINO baseline for `poisson_setup2`
#
# Hybrid data + physics baseline for the Poisson least-squares setup 2. The FC residual is evaluated pointwise with the FC_PINO derivative API, while the comparison residual and H(div) x H1 metrics are assembled from projected finite-element DoFs as in `train/poisson_setup2/poisson_setup2_fno.ipynb`.
...
model_train_outputs_path = repo_path / 'results/poisson_setup2/model_train_outputs/fc_pino_data_only'
model_test_outputs_path = repo_path / 'results/poisson_setup2/model_test_outputs/fc_pino_data_only'
...
# stalled training (loss stuck ~1.2); d=3 keeps it bounded and the loss decays normally.
fc_degree = 2
fc_cont_points = 50
n_modes = (32, 32)
hidden_channels = 128
