...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluate permeability on grid points.') 
    ...
    local_p_vertex_values = np.zeros((split_num_functions[rank], (mesh_args['num_x'] + 1) * (mesh_args['num_y'] + 1)), dtype=dtype)
    ...
    for i in tqdm(range(split_num_functions[rank])):
        ...
        p_fc.x.array[:] = local_p_dof[i]
        local_p_vertex_values[i][perm] = evaluate_expression(mesh, p_fc, mesh.geometry.x)[1][:, 0]

    gather_and_save(comm, dataset_path+'/p_vertex_values.npy', local_p_vertex_values, split_num_functions, dtype=dtype) 
    if rank == 0:
        print(f'Saved to {dataset_path}/p_vertex_values.npy')