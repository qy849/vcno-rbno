...
from utils import load_yaml, format_elapsed_time, load_and_scatter, gather_and_save, timing, convert_petsc_mat_to_torch_sparse_coo_tensor
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(split_num_functions[rank])):
        p = dolfinx.fem.Function(elasticity_least_squares.Vh['p'])
        p.x.array[:] = local_p_dof[i]
        weight = elasticity_least_squares.compute_weight(p)
