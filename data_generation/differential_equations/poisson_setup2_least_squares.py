...
class PoissonSetup2LeastSquares:
    ...
    def Vh(self):
        if self._Vh is None:
            ...
            m_element = basix.ufl.element(family=function_space_args["m"]["family"],
                                          cell=mesh_args["mesh_cell_type"],  
                                          degree=function_space_args["m"]["degree"])
    ...
    def f1(self):
        ...
        return dolfinx.fem.Constant(self.mesh, dolfinx.default_scalar_type((0.0, 0.0))) 

    def f2(self):
        mesh = self.mesh
        x = ufl.SpatialCoordinate(mesh)
        w = 1.0 / 32.0
        centers = [(m / 4.0, n / 4.0) for m in (1, 2, 3) for n in (1, 2, 3)]
        expr = 0
        for cx, cy in centers:
            expr += 100.0 * ufl.exp(-((ufl.sqrt((x[0] - cx) ** 2 + (x[1] - cy) ** 2) / w) ** 2))
        return expr
    ...
    def g(self):
        mesh = self.mesh
        x = ufl.SpatialCoordinate(mesh)
        return dolfinx.fem.Constant(self.mesh, dolfinx.default_scalar_type(0.0))
    ...
    def permeability(self, m: dolfinx.fem.Function): 
        return 0.01 + ufl.exp(m) # denoted as p
    ...
    def compute_L2_H1_norm(self, sigma, u):
        integral = ufl.inner(sigma, sigma) * ufl.dx + ufl.inner(u, u) * ufl.dx
        integral += ufl.inner(ufl.grad(u), ufl.grad(u)) * ufl.dx
        return np.sqrt(dolfinx.fem.assemble_scalar(dolfinx.fem.form(integral)))

    def compute_squared_L2_H1_norm(self, sigma, u):
        integral = ufl.inner(sigma, sigma) * ufl.dx + ufl.inner(u, u) * ufl.dx
        integral += ufl.inner(ufl.grad(u), ufl.grad(u)) * ufl.dx
        return dolfinx.fem.assemble_scalar(dolfinx.fem.form(integral))
...
if __name__ == '__main__':
    ...
    poisson_least_squares = PoissonSetup2LeastSquares(mesh_args=mesh_args, function_space_args=function_space_args)
    ...
    gather_and_save(comm, dataset_path+'/sigma_u_dof.npy', local_sigma_u_dof, split_num_functions, dtype=dtype)
    gather_and_save(comm, dataset_path+'/sigma_dof.npy', local_sigma_dof, split_num_functions, dtype=dtype)
    gather_and_save(comm, dataset_path+'/u_dof.npy', local_u_dof, split_num_functions, dtype=dtype)

    if rank == 0:
        print(f'Saved to {dataset_path}/sigma_u_dof.npy, {dataset_path}/sigma_dof.npy, {dataset_path}/u_dof.npy')