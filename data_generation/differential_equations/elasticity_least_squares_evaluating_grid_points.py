...
if __name__ == '__main__':
    ...
    local_sigma_u_dof, split_num_functions = load_and_scatter(comm, dataset_path+'/sigma_u_dof.npy',dtype=dtype)
    local_sigma_vertex_values = np.zeros((split_num_functions[rank], (mesh_args['num_x'] + 1) * (mesh_args['num_y'] + 1), 4), dtype=dtype)
    local_u_vertex_values = np.zeros((split_num_functions[rank], (mesh_args['num_x'] + 1) * (mesh_args['num_y'] + 1), 2), dtype=dtype)
    ...
    for i in tqdm(range(split_num_functions[rank]), desc="Evaluating sigma_u on grid points"):
        ...
        local_sigma_vertex_values[i, perm, 2] = sigma_grid_evals[:, 2]
        local_sigma_vertex_values[i, perm, 3] = sigma_grid_evals[:, 3]
        local_u_vertex_values[i, perm, 0] = u_grid_evals[:, 0]
        local_u_vertex_values[i, perm, 1] = u_grid_evals[:, 1]

    gather_and_save(comm, dataset_path+'/sigma_vertex_values.npy', local_sigma_vertex_values, split_num_functions, dtype=dtype)
    gather_and_save(comm, dataset_path+'/u_vertex_values.npy', local_u_vertex_values, split_num_functions, dtype=dtype)

    if rank == 0:
        print(f'Saved to {dataset_path}/sigma_vertex_values.npy and {dataset_path}/u_vertex_values.npy')