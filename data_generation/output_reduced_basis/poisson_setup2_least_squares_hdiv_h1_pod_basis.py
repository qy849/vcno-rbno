...
class Hdiv_H1_Norm(torch.nn.Module):
    def __init__(self, Vh, is_weight_tensor=False):
        super().__init__()
        self.Vh = Vh
        ...
        weight_1 = ufl.inner(sigma_trial, sigma_test) * ufl.dx + ufl.inner(u_trial, u_test) * ufl.dx
        weight_1 = dolfinx.fem.petsc.assemble_matrix(dolfinx.fem.form(weight_1))
        weight_1.assemble()
        self.weight_1 = weight_1

        weight_2 = ufl.inner(ufl.grad(u_trial), ufl.grad(u_test)) * dx
        weight_2 = dolfinx.fem.petsc.assemble_matrix(dolfinx.fem.form(weight_2))
        weight_2.assemble()
        self.weight_2 = weight_2

        weight_3 = ufl.inner(ufl.div(sigma_trial), ufl.div(sigma_test)) * dx
        ...
        if is_weight_tensor:
            ...
            self.weight_3 = convert_petsc_mat_to_torch_sparse_coo_tensor(weight_3, dtype=torch.float64)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate the POD basis of poisson solutions.')
    ...
    hdiv_h1_norm = Hdiv_H1_Norm(poisson_least_squares.Vh, is_weight_tensor=True)


    eigvals, pod_basis_dof = compute_pod_basis(snapshot, num_basis=output_reduced_basis_args['num_basis'], weight=hdiv_h1_norm.weight)
    ...
    pod_coeff_labels = torch.from_numpy(sigma_u_dof).to(torch_dtype) @ hdiv_h1_norm.weight @ pod_basis_dof
